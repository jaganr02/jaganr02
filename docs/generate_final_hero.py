...
# Load high-def dithered matrix portrait
with open('e:/github-portfolio/assets/dithered-portrait-hd.png', 'rb') as f:
    dither_b64 = base64.b64encode(f.read()).decode('utf-8')

svg_content = f'''<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="900" height="570" viewBox="0 0 900 570" fill="none" role="img" aria-label="R Jagan - AI Engineer &amp; Intelligent Systems Developer - RAG pipeline profile">
<defs>
  ...
  <filter id="d3" x="-30%" y="-30%" width="160%" height="170%">
    <feDropShadow dx="0" dy="8" stdDeviation="10" flood-color="#64748b" flood-opacity="0.22"/>
  </filter>

  <filter id="holoGlow" x="-20%" y="-20%" width="140%" height="140%">
    <feDropShadow dx="0" dy="0" stdDeviation="3" flood-color="#0284c7" flood-opacity="0.5"/>
  </filter>
  ...
  <clipPath id="portraitClip">
    <rect x="0" y="0" width="126" height="161" rx="10"/>
  </clipPath>
  ...
  <style>
    ...
    /* Strict Non-Overlapping Cycling Roles Animation */
    @keyframes roleCycle1 {{
      0%, 28% {{ opacity: 1; visibility: visible; transform: translateY(0px); }}
      33%, 95% {{ opacity: 0; visibility: hidden; transform: translateY(6px); }}
      100% {{ opacity: 1; visibility: visible; transform: translateY(0px); }}
    }}
    @keyframes roleCycle2 {{
      0%, 28% {{ opacity: 0; visibility: hidden; transform: translateY(-6px); }}
      33%, 61% {{ opacity: 1; visibility: visible; transform: translateY(0px); }}
      66%, 100% {{ opacity: 0; visibility: hidden; transform: translateY(6px); }}
    }}
    @keyframes roleCycle3 {{
      0%, 61% {{ opacity: 0; visibility: hidden; transform: translateY(-6px); }}
      66%, 95% {{ opacity: 1; visibility: visible; transform: translateY(0px); }}
    ...
    .holo-portrait {{
      animation: holoFlash 3.5s ease-in-out infinite;
    }}

    /* 3D Laser Scanline Sweep Beam */
    @keyframes laserSweep {{
      0% {{ transform: translateY(0px); opacity: 0; }}
      5% {{ opacity: 1; }}
      65% {{ transform: translateY(161px); opacity: 1; }}
      70% {{ opacity: 0; }}
    ...
    .pulse-dot {{ transform-origin: center; animation: pulse 2s infinite; }}
  </style>
</defs>

<!-- Outer Blueprint Canvas -->
...
<circle cx="820" cy="60" r="4" fill="#10b981" class="pulse-dot"/>

<!-- Running Vertical Flow Line from Search Bar to Node 1 -->
<line x1="200" y1="80" x2="200" y2="135" stroke="#0284c7" stroke-width="1.8" stroke-dasharray="4 4">
  <animate attributeName="stroke-dashoffset" from="0" to="-20" dur="1s" repeatCount="indefinite"/>
</line>

<!-- Running Pipeline Horizontal Stream Path -->
<path d="M 200 160 L 765 160" stroke="#0284c7" stroke-width="1.8" stroke-dasharray="5 5">
...
<circle r="4.5" fill="#0284c7">
  <animateMotion path="M 200 160 L 765 160" dur="3.5s" repeatCount="indefinite"/>
</circle>
...
<g transform="translate(490 134)">
  ...
  <text x="44" y="63" text-anchor="middle" class="mono" font-size="8.5" font-weight="600" fill="#64748b">4 · PROMPT CONSTRUCT</text>
  <text x="44" y="73" text-anchor="middle" class="mono" font-size="8.5" font-weight="600" fill="#64748b">(query + context)</text>
</g>
...
<!-- Running Vertical Connector Line from Final Answer to Streaming Terminal Card -->
<line x1="765" y1="184" x2="765" y2="235" stroke="#10b981" stroke-width="1.8" stroke-dasharray="4 4">
  <animate attributeName="stroke-dashoffset" from="0" to="-20" dur="1s" repeatCount="indefinite"/>
</line>
...
<g transform="translate(60 235)" filter="url(#d3)">
  ...
  <!-- Dynamic Non-Overlapping Alternating Roles (Poi Poi Varanum) -->
  <g transform="translate(24, 93)">
    <text x="0" y="0" class="sans role-1" font-size="15" font-weight="600" fill="#0284c7">AI Engineer · Generative AI &amp; Multi-Agent Systems</text>
    <text x="0" y="0" class="sans role-2" font-size="15" font-weight="600" fill="#7c3aed">Machine Learning Engineer · RAG &amp; Vector Search</text>
    <text x="0" y="0" class="sans role-3" font-size="15" font-weight="600" fill="#0d9488">Intelligent Systems Developer · FastAPI &amp; Python</text>
  </g>
  ...
  <!-- ================= RIGHT SIDE: 3D HOLOGRAPHIC DITHERED MATRIX PORTRAIT ================= -->
  <g transform="translate(615 16)">
    <text x="63" y="14" text-anchor="middle" class="mono" font-size="9" letter-spacing="2" fill="#64748b">&#9707; rendering subject&#8230;</text>
    
    <g transform="translate(0 20)">
      <!-- Frame Box -->
      <rect width="126" height="161" rx="10" fill="#f8fafc" stroke="#e2e8f0" stroke-width="1"/>
      
      <!-- Holographic Dithered Matrix Image with 3D Flash Shimmer -->
      <g class="holo-portrait" clip-path="url(#portraitClip)">
        <image href="data:image/png;base64,{dither_b64}" x="0" y="0" width="126" height="161" preserveAspectRatio="xMidYMid slice"/>
        
        <!-- 3D Laser Scanline Sweep Flash Effect -->
        <g class="laser-line">
          <line x1="0" y1="0" x2="126" y2="0" stroke="#0284c7" stroke-width="2.2" filter="url(#holoGlow)"/>
          <rect x="0" y="-12" width="126" height="12" fill="url(#laserGrad)" opacity="0.6"/>
        </g>
      </g>

      <!-- Status Pill Below Photo -->
      <g transform="translate(16 172)">
        <rect width="94" height="20" rx="10" fill="#ffffff" stroke="#cbd5e1" stroke-width="1"/>
...
print('PERFECT hero.svg generated! 100% visible portrait + clean non-overlapping cycling roles + 3D laser scan!')
