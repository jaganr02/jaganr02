...
sharp = ImageEnhance.Sharpness(img).enhance(2.2)
contrast = ImageEnhance.Contrast(sharp).enhance(1.25)
np_img = np.array(contrast)

# Precise silhouette mask: keep head, hair, face, neck, collar, suit
r = np_img[:, :, 0].astype(int)
g = np_img[:, :, 1].astype(int)
b = np_img[:, :, 2].astype(int)

# Background: green foliage (g > r+5 and g > b) and bright top sky
y_coords, x_coords = np.indices((h, w))
is_green = (g > r + 6) & (g > b + 2)
is_sky = (y_coords < int(h * 0.40)) & ((x_coords < int(w * 0.22)) | (x_coords > int(w * 0.78))) & ((r + g + b) / 3 > 90)
is_top_sky = (y_coords < int(h * 0.16)) & ((x_coords < int(w * 0.32)) | (x_coords > int(w * 0.68))) & ((r + g + b) / 3 > 90)

is_bg = is_green | is_sky | is_top_sky

alpha = np.where(is_bg, 0, 255).astype(np.uint8)
alpha_img = Image.fromarray(alpha).filter(ImageFilter.GaussianBlur(radius=0.8))

rgba = Image.new('RGBA', (w, h), (0, 0, 0, 0))
rgba.paste(contrast, (0, 0), alpha_img)
rgba.save('e:/github-portfolio/assets/jagan-hd-cutout.png')

with open('e:/github-portfolio/assets/jagan-hd-cutout.png', 'rb') as f:
    photo_b64 = base64.b64encode(f.read()).decode('utf-8')

# Build the complete SVG with:
# 1. Alternating Animated Titles (poi poi varanum)
# 2. Glowing Laser Scanline Loading Animation for the HD Photo
# 3. Flowing pipeline from search to card
# 4. Status pill jaganr02
...
<defs>
  ...
  <clipPath id="photoClip">
    <rect x="0" y="0" width="134" height="168" rx="10"/>
  </clipPath>

  <!-- Progressive Scanline Reveal Clip -->
  <clipPath id="scanRevealClip">
    <rect x="0" y="0" width="134" height="168">
      <animate attributeName="height" from="0" to="168" begin="1.8s" dur="1.6s" fill="freeze"/>
    </rect>
  </clipPath>
  ...
  <style>
    ...
    /* Smooth Looping Dynamic Role Transitions */
    ...
    @keyframes scanMove {{
      ...
      70% {{ transform: translateY(168px); opacity: 1; }}
      75%, 100% {{ transform: translateY(168px); opacity: 0; }}
    }}
    .laser-beam {{
...
<g transform="translate(60 235)" filter="url(#d3)">
  ...
  <!-- Dynamic Alternating Roles (poi poi varanum) -->
  ...
  <!-- ================= RIGHT SIDE: HIGH-DEFINITION CRYSTAL-CLEAR SCANNING PORTRAIT ================= -->
  <g transform="translate(600 16)">
    <text x="67" y="14" text-anchor="middle" class="mono" font-size="9" letter-spacing="2" fill="#64748b">&#9707; rendering subject&#8230;</text>
    
    <g transform="translate(0 20)">
      <!-- Frame Box -->
      <rect width="134" height="168" rx="10" fill="#f8fafc" stroke="#e2e8f0" stroke-width="1"/>
      
      <!-- Crystal-Clear HD Embedded Cutout Photo -->
      <g clip-path="url(#photoClip)">
        <!-- Progressive Top-to-Bottom Scanline Reveal -->
        <g clip-path="url(#scanRevealClip)">
          <image href="data:image/png;base64,{photo_b64}" x="0" y="0" width="134" height="168" preserveAspectRatio="xMidYMid slice"/>
        </g>
        
        <!-- Animated Glowing Laser Scanner Beam -->
        <g class="laser-beam">
          <line x1="0" y1="0" x2="134" y2="0" stroke="#0284c7" stroke-width="2.5"/>
          <rect x="0" y="-14" width="134" height="14" fill="url(#scanBeam)"/>
        </g>
      </g>
...
print('ULTIMATE hero.svg successfully created and validated with crystal-clear photo + laser scan + animated cycling roles!')
