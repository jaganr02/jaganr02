...
# High density 52 cols x 48 rows
cols = 52
rows = 48
...
def get_char_and_color(r, g, b, x_idx, y_idx):
    ...
    is_corner_bg = (y_idx < 16 and (x_idx < 10 or x_idx > 40)) or (y_idx < 8 and (x_idx < 14 or x_idx > 36))
    ...
    if luminance > 215:
        ...
        return ('&#9608;', '#0f766e') # Deep cyan/teal block
...
for row_idx in range(rows):
    ...
    for col_idx in range(cols):
        ...
        if res is not None:
            ...
            if color == current_color and run_start_x is not None:
                ...
                if current_char_run:
                    text_len = round(char_count * 1.8, 1)
    ...
    if current_spans:
        ...
        # Looping progressive holographic reveal animation + initial scan
        line_svg = f'<text y="{round(y_pos, 1)}" opacity="0">{spans_str}<animate attributeName="opacity" from="0" to="1" begin="{round(begin_t, 2)}s" dur="0.25s" fill="freeze"/></text>'
...
<defs>
  ...
  <linearGradient id="laserGrad" x1="0" y1="0" x2="0" y2="1">
    ...
    <stop offset="85%" stop-color="#38bdf8" stop-opacity="0.35"/>
  ...
  <filter id="holoGlow" x="-20%" y="-20%" width="140%" height="140%">
    <feDropShadow dx="0" dy="0" stdDeviation="3" flood-color="#0284c7" flood-opacity="0.4"/>
  </filter>
  ...
  <style>
    ...
    /* Smooth Looping Dynamic Role Transitions (Poi Poi Varanum) */
    ...
    .holo-matrix {{
      animation: holoFlash 3s ease-in-out infinite;
    }}

    /* 3D Laser Scanner Sweep Line */
    @keyframes laserSweep {{
      0% {{ transform: translateY(0px); opacity: 0; }}
      5% {{ opacity: 1; }}
      70% {{ transform: translateY(165px); opacity: 1; }}
      75% {{ opacity: 0; }}
...
<g transform="translate(60 235)" filter="url(#d3)">
  ...
  <!-- ================= RIGHT SIDE: 3D HOLOGRAPHIC SCANNING PORTRAIT ================= -->
  <g transform="translate(680 14)">
    <text x="56" y="16" text-anchor="middle" class="mono" font-size="9" letter-spacing="2" fill="#64748b">&#9707; rendering subject&#8230;</text>
    
    <!-- Holographic Scanning Matrix -->
    <g class="mono holo-matrix" font-size="3.35">
      {matrix_xml}
    </g>

    <!-- 3D Laser Scanline Flash Effect -->
    <g class="laser-line" transform="translate(14 280)">
      <line x1="0" y1="0" x2="96" y2="0" stroke="#0284c7" stroke-width="2" filter="url(#holoGlow)"/>
      <rect x="0" y="-12" width="96" height="12" fill="url(#laserGrad)" opacity="0.6"/>
    </g>
...
print('FLASH 3D hero.svg successfully created with progressive holographic scan + laser beam + running lines + cycling roles!')
